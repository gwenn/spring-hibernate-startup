...
import org.hibernate.persister.LazyPersisterFactory;
...
public final class SessionFactoryImpl implements SessionFactory, SessionFactoryImplementor {
	...
	private boolean lazy = true; // FIXME
   ...
   public SessionFactoryImpl(
	...
	throws HibernateException {
		...
		while ( classes.hasNext() ) {
			...
			EntityPersister cp;
			if (lazy) {
				cp = LazyPersisterFactory.createClassPersister(model, cache, this, mapping);
			} else {
				cp = PersisterFactory.createClassPersister(model, cache, this, mapping);
			}
			if ( cache != null && cache.getCache() instanceof OptimisticCache ) {
				if (lazy) { // FIXME
					throw new IllegalStateException("Lazy mode...");
				}
		...
		while ( collections.hasNext() ) {
			...
			if (lazy) {
				persister = LazyPersisterFactory.createCollectionPersister(cfg, model, cache, this);
			} else {
				persister = PersisterFactory.createCollectionPersister(cfg, model, cache, this);
			}
	...
	public EntityPersister getEntityPersister(String entityName) throws MappingException {
		...
		result = replaceLazyProxy(entityName, result);
		return result;
	}
	...
	public CollectionPersister getCollectionPersister(String role) throws MappingException {
		...
		result = replaceLazyProxy(role, result);
		return result;
	}
	...
	public CollectionMetadata getCollectionMetadata(String roleName) throws HibernateException {
		CollectionMetadata collectionMetadata = (CollectionMetadata) this.collectionMetadata.get(roleName);
		collectionMetadata = (CollectionMetadata)replaceLazyProxy(roleName, (CollectionPersister)collectionMetadata);
		return collectionMetadata;
	}

	public ClassMetadata getClassMetadata(String entityName) throws HibernateException {
		ClassMetadata classMetadata = (ClassMetadata) this.classMetadata.get(entityName);
		classMetadata = (ClassMetadata)replaceLazyProxy(entityName, (EntityPersister)classMetadata);
	...
	private EntityPersister replaceLazyProxy(String entityName, EntityPersister persister) {
		if (lazy && LazyPersisterFactory.isLazyProxy(persister)) {
			persister = LazyPersisterFactory.get(persister); // TODO Synchronize for thread safety
			entityPersisters.put(entityName, persister);
			classMetadata.put(entityName, persister.getClassMetadata());
			persister.postInstantiate();
		}
		return persister;
	}
	private CollectionPersister replaceLazyProxy(String role, CollectionPersister persister) {
		if (lazy && LazyPersisterFactory.isLazyProxy(persister)) {
			persister = LazyPersisterFactory.get(persister); // TODO Synchronize for thread safety
